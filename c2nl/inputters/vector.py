...
def vectorize(ex, model):
    ...
    vectorized_ex['ast_distance'] = None
    vectorized_ex['ast_rpe'] = None
    ...
    if code.struc is not None:
        ...
        vectorized_ex['ast_distance'] = torch.LongTensor(code.ast)
        vectorized_ex['ast_rpe'] = torch.LongTensor(code.arpe)
...
def batchify(batch):
    ...
    ast_distance = [ex['ast_distance'] for ex in batch]
    ast_rpe = [ex['ast_rpe'] for ex in batch]
    ...
    ast_distance_rep = torch.zeros(batch_size, max_code_len, max_code_len, dtype=torch.float) \
        if use_code_struc else None
    ast_rpe_rep = torch.zeros(batch_size, max_code_len, max_code_len, dtype=torch.long) \
        if use_code_struc else None
    ...
    for i in range(batch_size):
        ...
        if use_code_struc:
            ...
            ast_distance_rep[i, :, :].copy_(ast_distance[i][:max_code_len, :max_code_len])
            ast_rpe_rep[i, :, :].copy_(ast_rpe[i][:max_code_len, :max_code_len])
