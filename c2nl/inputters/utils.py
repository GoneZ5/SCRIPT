...
def process_examples(lang_id,
    ...
    if guid is not None:
        if lang_id == 0:
            code.struc = np.load('../../data/java/adjacency/{}.npy'.format(guid))
            y = np.load('../../data/java/distance/{}.npy'.format(guid))
            code.arpe = y.copy()
            y = y.astype(np.float16)
            if length > 1:
                y = np.reciprocal((y[: length, : length] + 0.01).astype(np.float16))
                row, col = np.diag_indices_from(y)
                y[row, col] = 0
                code.ast = y / y.sum(axis=1, keepdims=True)
                if length < 512:
                    code.ast = np.pad(code.ast, (0, 512 - length), 'constant')
            else:
                code.ast = y
        else:
            code.struc = np.load('../../data/python/adjacency/{}.npy'.format(guid))
            y = np.load('../../data/python/distance/{}.npy'.format(guid))
            code.arpe = y.copy()
            y = y.astype(np.float16)
            if length > 1:
                y = np.reciprocal((y[: length, : length] + 0.01).astype(np.float16))
