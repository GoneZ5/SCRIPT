...
class Encoder(nn.Module):
    ...
    def forward(self,
                ...
                ast_rpe,
                ast_distance):
        layer_outputs, _ = self.transformer(input, input_len, adjacency, ast_rpe, ast_distance)  # B x seq_len x h
        if self.use_all_enc_layers:
...
#        depth = kwargs['depth']
        memory_bank, layer_wise_outputs = self.encoder(code_rep, code_len, code_struc_rep, ast_rpe, ast_distance)  # B x seq_len x h
    ...
    def decode(self,
        ...
        ast_rpe = kwargs['ast_rpe']
#        depth = kwargs['depth']
        memory_bank, layer_wise_outputs = self.encoder(word_rep, code_len, code_struc_rep, ast_rpe, ast_distance)  # B x seq_len x h
