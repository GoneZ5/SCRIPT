...
from c2nl.utils.misc import generate_relative_positions_matrix, \
    relative_matmul, relative_matmul_ast
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None, ast_rpe=None, layer_cache=None,
        ...
        if layer_cache is not None:
            ...
            key = shape(self.key(key), self.d_k)  # [B, key_len, 512] -> [[B, 8, key_len, 64]]
            value = shape(self.value(value), self.d_v)
            query = shape(self.query(query), self.d_k)

        if self.max_relative_positions > 0 and attn_type == "self":  # 64
            key_len = key.size(2)
            ...
            relations_keys = self.relative_positions_embeddings_k(  # [34, 34, 64]
        ...
        if self.max_relative_positions > 0 and attn_type == "self":
            context = unshape(context_original + relative_matmul(drop_attn, relations_values, False), self.d_v)
        else:
...
class MultiHeadedAttention_arpe(nn.Module):
    ...
    def __init__(self, head_count, model_dim, d_k, d_v, dropout=0.1,
        ...
        if max_relative_positions > 0:
            ...
            self.ast_positions_embeddings_k = nn.Embedding(
                97, self.d_k)  # java 97  python 32
            self.relative_positions_embeddings_v = nn.Embedding(
                vocab_size, self.d_v)
            self.ast_positions_embeddings_v = nn.Embedding(
                97, self.d_v)
    ...
    def forward(self, key, value, query, mask=None, ast_rpe=None, layer_cache=None,
        ...
        if self.max_relative_positions > 0 and attn_type == "self":
            # ast结点距离相对位置编码key
            scores = query_key + relative_matmul(query, relations_keys, True) + relative_matmul_ast(query, ast_keys, True)
        else:
            scores = query_key
        scores = scores.float()
        ...
        if self.max_relative_positions > 0 and attn_type == "self":
            context = unshape(context_original + relative_matmul(drop_attn, relations_values, False)
                              + relative_matmul_ast(drop_attn, ast_values, False), self.d_v)
        else:
            context = unshape(context_original, self.d_v)
